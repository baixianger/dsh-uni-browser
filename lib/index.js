...
import { UniBrowserRuntime } from "./runtime.js";

export { PLATFORM_PACKAGES, UNI_BROWSER_RUNTIME_VERSION, UniBrowserRuntime, platformPackageName } from "./runtime.js";
...
export const Config = Schema.object({
  ...
  defaultEngine: Schema.string().default("chromium"),
  root: Schema.string(),
  path: Schema.string(),
...
export class DshUniBrowser {
  ...
  async call(action, session = "default", params = {}) { if (this.runtime) await this.runtime.ensure(this.client); return this.client.call(action, session, params); }
  async health() { const data = await this.call("daemon.ping"); return { online: Boolean(data.pong), socketPath: this.client.socketPath, managed: Boolean(this.runtime?.managed), runtimeSource: this.runtime?.runtimeSource ?? "injected-client" }; }
  async profiles() {
    const profiles = await this.store.list(); let active = new Set(); let online = false;
    try { const data = await this.call("session.list"); active = new Set((data.sessions ?? []).map(String)); online = true; } catch {}
  ...
  async open(id) {
    ...
    const data = await this.call("session.create", profile.session, { engine: profile.engine, headless: profile.headless, user_data_dir: profileDir, audit: true }); return { profile, data };
  }
  async close(id) { const profile = await this.store.get(id); const data = await this.call("session.close", profile.session, { purge: false }); return { profile, data }; }
  async forget(id, confirm) { if (confirm !== true) throw new Error("forgetting a profile permanently deletes its login state; pass confirm: true"); const profile = await this.store.get(id); try { await this.call("session.close", profile.session, { purge: false }); } catch {} return this.store.forget(profile.id); }
  async action(id, action, params = {}) { const profile = await this.store.get(id); return this.call(action, profile.session, params); }
}
...
export function apply(ctx, config) {
  ...
  ctx.inject(["webServer"], (webCtx) => {
    webCtx.connection.rpc.handle("/dsh-uni-browser", async (endpoint, payload) => {
